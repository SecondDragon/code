...
function render(vdom, container) {
    ...
    scheduleUpdate = () => {
        hookIndex = 0;
        //更新的关键
        //vdom并不指向当前的更新，而是指向根元素
        // react每次更新都是从根部开始对比，这时两个vdom是同一个，然后因为类组件的vdom有类的实例，可以检测出来是类组件的vdom节点，
        // 然后从新执行实例的render，得到新的vdom从新调用compareTwoVdom 这时就有新旧对比了
        //函数式组件类似 vdom的type是函数的实例，发现是函数式Vdom节点，就执行得到新的vdom,然后再调用compareTwoVdom更新
...
export function useLayoutEffect(callback, deps) {
    if (hookState[hookIndex]) {
        ...
        //初次渲染的时候，开启一个微任务，在宏任务里执行callback,保存销毁函数和依赖数组
        queueMicrotask(() => {
            let destroy = callback();
            hookState[hookIndex++] = [destroy, deps];
        });
...
function createDOM(vdom) {
    ...
    if (props) {
        ...
        } else if (Array.isArray(props.children)) {//如果是一个数组 就说明有多个儿子，

            reconcileChildren(props.children, dom);
        }
    }

    //当本组件全部渲染完毕后，将 mountingComponent 重新置为 null
...
function mountClassComponent(vdom) {
    ...
    //暂时把didMount方法暂存到dom上 以便真正挂载的之后调用
...
function mountFunctionComponent(vdom) {
    let {type, props} = vdom;
    let renderVdom = type(props);//对函数式组件，要执行他拿到真正的 虚拟dom
    vdom.oldRenderVdom = renderVdom;
    return createDOM(renderVdom);
}
