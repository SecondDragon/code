...
function initWatch (vm: Component, watch: Object) {
  for (const key in watch) { // 循环所有的watch
...
function createWatcher (
  ...
  if (isPlainObject(handler)) { // 有可能是对象类型 watch:{aaa:{handler(){},immediate:true)
    options = handler
    handler = handler.handler
  }
  if (typeof handler === 'string') { // watch可以是一个字符串
    handler = vm[handler]
  }
...
export function stateMixin (Vue: Class<Component>) {
  ...
  Vue.prototype.$watch = function (
    ...
    return function unwatchFn () { // 返回一个取消观测方法
      watcher.teardown()
    }
  }
}
